...
export default function CampaignNew() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      setErrors({ general: message ?? 'Failed to create campaign. Please try again.' })
    },
  })
  ...
  function validate(): FormErrors {
    ...
    if (selectedIds.size === 0) errs.recipients = 'Select at least one recipient.'
    return errs
  }
  ...
  async function handleQuickAdd() {
    ...
    try {
      ...
      // Refetch recipients and auto-select the new one
  ...
  const someSelected = selectedIds.size > 0 && selectedIds.size < recipients.length
        ...
        <div className="card p-6 space-y-4">
          ...
          {loadingRecipients ? (
            ...
            <p className="text-sm text-gray-500 text-center py-6">
              {search ? 'No recipients match your search.' : 'No recipients yet. Add one above.'}
            </p>
          ) : (
