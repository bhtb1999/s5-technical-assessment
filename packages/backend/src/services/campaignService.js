...
async function processSend(campaign) {
  try {
    ...
    try {
      ...
      console.error(
        "Failed to update campaign status after send error:",
        innerErr,
      );
    }
  }
...
async function processScheduledCampaigns() {
  ...
  if (dueCampaigns.length > 0) {
    console.log(
      `Scheduler: found ${dueCampaigns.length} campaign(s) due to send.`,
    );
  ...
  if (stuckCampaigns.length > 0) {
    console.log(
      `Scheduler: recovering ${stuckCampaigns.length} stuck campaign(s).`,
    );
