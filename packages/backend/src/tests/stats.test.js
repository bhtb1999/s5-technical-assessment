...
async function createCampaignWithStats({
  sent = 0,
  failed = 0,
  opened = 0,
} = {}) {
  ...
  if (total > recipientIds.length)
    throw new Error("Not enough test recipients");
...
describe("Stats accuracy after background send completes", () => {
  it("all CampaignRecipient records should be non-pending after send completes", async () => {
    ...
    const totalAccountedFor = statsRes.body.sent + statsRes.body.failed;
